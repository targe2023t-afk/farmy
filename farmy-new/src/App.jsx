...
export default function App() {
  ...
  useEffect(()=>{
    ...
    // ✅ Fix: استقبال deep link على Android بعد Google OAuth
      ...
      try {
        ...
        CapApp.addListener("appUrlOpen", async ({ url }) => {
          if(url?.startsWith("com.farmy.app://login-callback")){
            await Browser.close(); // أغلق الـ browser
            // استنى supabase يستقبل الـ session
            const { data: { session } } = await supabase.auth.getSession();
            if(session?.user) await handleOAuthUser(session.user);
          }
        });
      } catch(_){}
    };
...
function LoginPage({users,setUsers,onLogin}) {
  ...
  const doGoogleLogin=async()=>{
    ...
    try{
      ...
      if(isNative){
        // ✅ Fix: على Android/iOS استخدم Capacitor Browser عشان Google OAuth يشتغل
        const { Browser } = await import("@capacitor/browser");
        const redirectTo = "com.farmy.app://login-callback";
        const { data, error } = await supabase.auth.signInWithOAuth({
          provider: 'google',
          options: { redirectTo, skipBrowserRedirect: true },
        });
        if(error){ setErr(error.message); setBusy(false); return; }
        // افتح Google في Browser مستقل مش WebView
      ...
      setErr("خطأ في تسجيل الدخول بـ Google: " + e.message);
    }
    setBusy(false);
  };
  ...
  return (
    <div className="login-wrap" dir="rtl" style={{fontFamily:"'Cairo',sans-serif",position:"relative",overflow:"hidden"}}>
      ...
      <div className="login-top" style={{textAlign:"center",paddingBottom:20}}>
        {/* New logo */}
        <img src="/icons/icon.png" className="login-logo-new" onError={e=>{e.target.style.display="none";}} style={{width:96,height:96,borderRadius:28,objectFit:"cover",boxShadow:"0 8px 28px rgba(13,99,27,.28)"}} alt="farmy"/>
        <div className="login-title">farmy</div>
